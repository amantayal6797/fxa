...
export class StripeWebhookHandler extends StripeHandler {
  ...
  async handlePlanUpdatedEvent(request: AuthRequest, event: Stripe.Event) {
    ...
    const products = await this.stripeHelper.fetchAllProducts();
    let product = products.find((p) => p.product_id === plan.product);
    if (!product) {
      this.log.error(
  ...
  async handleProductUpdatedEvent(request: AuthRequest, event: Stripe.Event) {
    ...
    for await (const item of plans) {
      ...
      if (result?.error) {
        ...
        this.log.error(msg, { item: product, error: result.error });
